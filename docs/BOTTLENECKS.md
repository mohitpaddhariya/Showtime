...
| AI Mapping | Groq LLM API | ~1s | ~1.5s | ~2s | ~3s |
| Rendering | FFmpeg | ~5s | ~20s | ~40s | ~2min |
| **Total (Groq)** | | **~10s** | **~35s** | **~1min** | **~3min** |
...
## Bottleneck 2: Scene Detection (OpenCV)
...
- **Auto-split**: segments > `MAX_SEGMENT_DURATION` (10s) re-scanned at lower threshold (20 vs 30)
...
SHOWTIME_MAX_SEGMENT_DURATION=10     # auto-split above this
...
|----------|-------|--------|
| Vision model (Llama 4 Scout) | 15/min, 7,000/day | 1 per video — not a bottleneck |
| Text model (Llama 3.3 70B) | 30/min, 14,400/day | 1 per video (fallback only) |
| Whisper requests | 20/min, 7,200/day | 1 per video — not a bottleneck |
| Whisper audio | 28,800 sec/day | = 480 min/day of audio |
...
LLM context is not a limit even for 1hr+ videos.
...
| Use case | Whisper | LLM | Preset | Processing time (10 min video) |
|----------|---------|-----|--------|-------------------------------|
| **Fast dev** | Groq | Groq | ultrafast | ~30s |
| **Production** | Groq | Groq | medium | ~1 min |
| **Offline** | local (base) | Ollama | medium | ~6 min |
| **Best quality** | Groq (large-v3) | Groq (70b) | slow | ~2 min |
